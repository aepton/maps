...
const Map = ({ zoom, center, minZoom, maxZoom }) => {
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
        const response = await fetch('/static/streets_heights.geojson');
        if (!response.ok) {
...
export const Head = () => <title>Streets in Seattle</title>
